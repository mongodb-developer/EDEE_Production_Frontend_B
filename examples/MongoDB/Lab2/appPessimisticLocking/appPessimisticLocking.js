...
async function get_EditableRecord(req, res) {
  ...
  var checkIfLocked = { _id: newDocId, isLocked: null }
  var takeLock = { $set: { isLocked: true, lockId: lockId } }

  var docToEdit = null;

  updateResult = await collection.updateOne(checkIfLocked, takeLock, { returnNewDocument: true })
...
async function post_EditableRecord(req, res) {
  ...
  // Check it's locked and we have the id
  var query = { _id: newVersion._id, isLocked: true, lockId: newVersion.lockId }
  var update = { $set: { content: newVersion.content }, $unset: { isLocked: true, lockId: true } }
  ...
  if (updateResult.matchedCount == 1) {
    ...
    postUpdate = { error: "You did not have the lock for that" }
  }
  res.status(201);
...
async function initWebService() {
  ...
  try {
    await collection.insertOne({ _id: newDocId, content: "A document people edit by hand" })
