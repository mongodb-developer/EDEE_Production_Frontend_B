...
async function get_Dashboard(req, res) {
    ...
    var groupByCountryAndMarket = {$group : {
        _id : { country: "$address.country", market: "$address.market" },
        nBeds : { $sum : "$beds" },
        totalPrice : {$sum : "$price"}
    }}

    var pricePerBed =  { $set : { pricePerBed : {$divide: [ "$totalPrice","$nBeds"]}}}
    ...
    var groupByCountry = { $group : {
        ...
        topMarkets : { $topN : {
            n: 3,
            sortBy: { nBeds: -1 },
            output: { market: "$_id.market",beds:"$nBeds",pricePerBed: "$pricePerBed"}
        }}
    }}
    ...
    var tidyUp = { $set: { Country: "$_id", _id: "$$REMOVE", totalBeds: "$$REMOVE"}}
