...
async function post_AtlasSearch(req, res) {
  ...
  try {
    ...
    rval.drop =
      "Index cannot be dropped - perhaps does not exist\n" + e.toString();
  }
  ...
  try {
    ...
    rval.index =
      "Index cannot be created, possibly still being dropped " + e.toString();
  }
...
async function get_AtlasSearch(req, res) {
  ...
  searchOperation = [
    { $search: { text: { query: queryTerm, path: { wildcard: "*" } } } },
  ];
