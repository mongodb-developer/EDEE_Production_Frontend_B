...
var taskCollection
const STATUS_NEW="1 - New", STATUS_ASSIGNED="2 - Assigned", STATUS_DONE="3 - Complete"
...
async function post_Assign(req,res) {
    ...
    if(! assignedTo) { res.status(400); res.send({error:"No user specified"});return;}
    oneMinuteAgo = new Date()
    oneMinuteAgo.setMinutes(oneMinuteAgo.getMinutes()-1)

    // Answer here you may need { $or : [ {query1}, {query2} ]}
    newTasks = { status: STATUS_NEW  }
    abandonedTasks = { status: STATUS_ASSIGNED, dateAssigned : {$lt: oneMinuteAgo} }
    assignableTasks = { $or : [newTasks,abandonedTasks]}

    assignTask = { $set : { status: STATUS_ASSIGNED, assignedTo, dateAssigned: new Date()}}
    
    // Sort to prioritise status 2 (Abandoned)
    options = {returnNewDocument : true, sort: {status: -1}}
    
    assignedTask = await taskCollection.findOneAndUpdate(assignableTasks,assignTask,options)
    res.status(202)
...
async function post_Task(req, res) {
    ...
    const description = "This doesn't matter in this exercise but you would send it to the API"
    newTask = {date: new Date(), status: STATUS_NEW, assignedTo : null, dateAssigned: null, description }
...
async function post_Complete(req,res) {
  taskId = req.params[3]
  if(! taskId) { res.status(400); res.send({error:"No task specified"});return;}
